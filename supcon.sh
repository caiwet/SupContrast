...
#SBATCH -o supcon_left_right%j.out                 # File to which STDOUT will be written, including job ID (%j)
#SBATCH -e supcon_left_right%j.err                 # File to which STDERR will be written, including job ID (%j)
...
python main_supcon.py --batch_size 32  --epochs 100 \
--learning_rate 0.01  --lr_decay_epochs 20,30,40,60 \
--temp 0.5   --cosine --dataset path --mean "(0.485, 0.456, 0.406)" \
--std "(0.229, 0.224, 0.225)" \
--data_folder /n/groups/patel/Alan/Aging/Medical_Images/images/Arterial/Carotids/CIMT150 --size 128
